function Export-CostData {
    ...
    .DESCRIPTION
        Retrieving cost data from Azure can take a few minutes to run. Export-CostData lets you persist the result to disk
    ...
    .PARAMETER Path
        The file to save the cost data to. If a directory is specified instead, a file named CostData_<timestamp>.json is created within it.
    ...
    process {
        ...
        if (Test-Path $Path -PathType Container) {
            $Path = Join-Path $Path "CostData_$(Get-Date -Format 'yyyyMMddHHmmss').json"
        }
